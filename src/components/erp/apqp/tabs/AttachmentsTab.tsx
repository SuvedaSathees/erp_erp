...
export const AttachmentsTab: React.FC<AttachmentsTabProps> = ({
  ...
  const [previewAttachment, setPreviewAttachment] = useState<ApqpAttachment | null>(null);
  ...
  const handleDownload = (attachment: ApqpAttachment) => {
    const blob = new Blob(
      [
        `Magnertia ERP APQP Controlled Quality Document\n\nTitle: ${attachment.documentType}\nFile: ${attachment.fileName}\nVersion: ${attachment.version}\nUploaded By: ${attachment.uploadedBy}\nDate: ${attachment.uploadedDate}\nSize: ${attachment.fileSize}\nStatus: Verified Quality Gate Baseline\n\nAPQP Reference: ${record.apqpNumber} (${record.apqpProjectName})\nCustomer: ${record.customer}\nProduct: ${record.product} (${record.productRevision})`,
      ],
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-base font-bold text-foreground">
                APQP Attachments & Controlled Quality Documents Repository
              </CardTitle>
            </div>
            <p className="text-xs text-muted-foreground mt-0.5">
              Central repository for APQP checklists, DFMEA, PFMEA, Control Plans, PPAP packages, and customer specifications.
            </p>
          </div>
          <Button
            size="sm"
      ...
      <Dialog open={!!previewAttachment} onOpenChange={(open) => !open && setPreviewAttachment(null)}>
        <DialogContent className="max-w-xl">
          ...
          <div className="p-4 rounded-lg bg-slate-900 text-slate-100 font-mono text-xs max-h-72 overflow-y-auto space-y-2">
            <p className="text-emerald-400 font-bold">=== APQP QUALITY GATE SPECIFICATION ===</p>
            ...
            <p>APQP Ref: {record.apqpNumber} ({record.apqpProjectName})</p>
            <div className="mt-3 p-3 rounded bg-slate-800/80 text-slate-300 font-sans text-xs">
              This quality document establishes verified gate deliverables and customer compliance verification for {record.product} ({record.productRevision}).
            </div>
          </div>
