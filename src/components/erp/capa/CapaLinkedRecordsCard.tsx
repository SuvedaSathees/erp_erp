...
import { Link2, ExternalLink, AlertOctagon, HelpCircle, ShieldCheck, CheckSquare } from "lucide-react";
import { CapaRecord } from "@/services/capaTypes";
import { Link } from "@tanstack/react-router";

interface CapaLinkedRecordsCardProps {
  record: CapaRecord;
}

export function CapaLinkedRecordsCard({ record }: CapaLinkedRecordsCardProps) {
  return (
    <Card className="shadow-xs border-border/80">
      ...
      <CardContent className="pt-3 p-0">
        <div className="divide-y divide-border/60 text-xs">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <span className="text-[11px] text-muted-foreground block">
                  Wave solder bridging non-conformance
                </span>
              </div>
            </div>
          ...
          {record.linkedRcaId && (
            <Link
              to="/management/quality-management/root-cause-analysis"
              className="p-3 hover:bg-muted/30 transition-colors flex items-center justify-between group"
            >
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <span className="text-[11px] text-muted-foreground block">
                    5-Why & Ishikawa fishbone analysis
                  </span>
                </div>
              </div>
              <ExternalLink className="w-3.5 h-3.5 text-muted-foreground group-hover:text-primary" />
          ...
          >
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <span className="text-[11px] text-muted-foreground block">
                  Q3 Quality & Process Audit finding
                </span>
              </div>
            </div>
            <ExternalLink className="w-3.5 h-3.5 text-muted-foreground group-hover:text-primary" />
