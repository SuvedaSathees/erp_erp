...
interface ControlPlanHeaderProps {
  record: ControlPlanRecord;
  onSaveDraft: () => void;
  onSubmitForReview: () => void;
  onExport: () => void;
  onNewControlPlan: () => void;
}

export const ControlPlanHeader: React.FC<ControlPlanHeaderProps> = ({
  record,
  onSaveDraft,
  onSubmitForReview,
  onExport,
        ...
        <div className="space-y-1.5 min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className="px-2.5 py-0.5 rounded bg-muted font-mono font-semibold text-xs border border-border whitespace-nowrap">
              {record.controlPlanId}
            </span>
            <h1 className="text-base font-bold text-foreground truncate">
              {record.controlPlanTitle}
            </h1>
            <Badge
              variant="outline"
