...
import type { JigApprovalDecision, JigFormInput, JigReviewer } from "@/services/types";
import { jigDevelopmentService } from "@/services/jigDevelopmentService";
import { toast } from "sonner";

export function JigApprovalSection({
  form,
  reviewers,
}: {
  ...
  const [decision, setDecision] = useState<JigApprovalDecision>("Approved");
  ...
  const handleApplyDecision = async () => {
    ...
    try {
      const res = await jigDevelopmentService.reviewDecision({
      ...
      <CardHeader className="pb-3 flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription className="text-xs">
            Cross-functional engineering sign-offs: Design, Manufacturing, Quality, Maintenance, Plant Head, COO, CEO.
          </CardDescription>
        </div>

        <Badge className="bg-blue-600 text-white text-xs font-semibold px-3 py-1">
          Workflow Stage: Review & Approval
        </Badge>
      </CardHeader>
