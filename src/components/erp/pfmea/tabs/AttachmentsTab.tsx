...
export const AttachmentsTab: React.FC<AttachmentsTabProps> = ({
  ...
  const [previewAttachment, setPreviewAttachment] = useState<PfmeaAttachment | null>(null);
  ...
  const handleDownload = (attachment: PfmeaAttachment) => {
    const blob = new Blob(
      [
        `Magnertia ERP PFMEA Controlled Document\n\nTitle: ${attachment.documentType}\nFile: ${attachment.fileName}\nVersion: ${attachment.version}\nUploaded By: ${attachment.uploadedBy}\nDate: ${attachment.uploadedDate}\nSize: ${attachment.fileSize}\nStatus: Verified AIAG-VDA Harmonized Baseline\n\nPFMEA Reference: ${record.pfmeaNumber} (${record.pfmeaTitle})\nProduct: ${record.product} (${record.productRevision})\nManufacturing Process: ${record.manufacturingProcess}`,
      ],
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-base font-bold text-foreground">
                PFMEA Controlled Attachments & Documents Repository
              </CardTitle>
            </div>
            <p className="text-xs text-muted-foreground mt-0.5">
              Centralized document management for PFMEA Worksheets, Process Flow Diagrams, Control Plans, Work Instructions, and Validation Reports.
            </p>
          </div>
          <Button
            size="sm"
      ...
      <Dialog open={!!previewAttachment} onOpenChange={(open) => !open && setPreviewAttachment(null)}>
        <DialogContent className="max-w-xl">
          ...
          <div className="p-4 rounded-lg bg-slate-900 text-slate-100 font-mono text-xs max-h-72 overflow-y-auto space-y-2">
            <p className="text-emerald-400 font-bold">=== PFMEA CONTROLLED SPECIFICATION ===</p>
            ...
            <p>PFMEA Ref: {record.pfmeaNumber} ({record.pfmeaTitle})</p>
            <div className="mt-3 p-3 rounded bg-slate-800/80 text-slate-300 font-sans text-xs">
              This controlled document certifies that potential failure modes, causes, effects, and mitigation controls have been evaluated per AIAG-VDA FMEA standards under APQP Gate 3.
            </div>
          </div>
