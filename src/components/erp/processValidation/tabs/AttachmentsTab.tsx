...
export const AttachmentsTab: React.FC<AttachmentsTabProps> = ({
  ...
  const [previewAttachment, setPreviewAttachment] = useState<ProcessValidationAttachment | null>(null);
      ...
      <Dialog open={!!previewAttachment} onOpenChange={(open) => !open && setPreviewAttachment(null)}>
        <DialogContent className="max-w-xl">
          ...
          <div className="p-4 rounded-lg bg-slate-900 text-slate-100 font-mono text-xs max-h-72 overflow-y-auto space-y-2">
            <p className="text-emerald-400 font-bold">=== PROCESS VALIDATION CONTROLLED DOCUMENT ===</p>
            ...
            <p>Process: {record.manufacturingProcess} ({record.productionLine})</p>
            <p>Validation Ref: {record.validationNumber} ({record.validationTitle})</p>
            <div className="mt-3 p-3 rounded bg-slate-800/80 text-slate-300 font-sans text-xs">
              This controlled document certifies that the manufacturing process ({record.manufacturingProcess}) operates within specified statistical tolerance limits under APQP & PPAP guidelines.
            </div>
          </div>
