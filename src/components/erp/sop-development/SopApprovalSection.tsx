...
import type { SopApprovalDecision, SopFormInput, SopReviewer } from "@/services/types";
import { sopDevelopmentService } from "@/services/sopDevelopmentService";
import { toast } from "sonner";

export function SopApprovalSection({
  form,
  reviewers,
}: {
  ...
  const [decision, setDecision] = useState<SopApprovalDecision>("Approved");
  ...
  const handleApplyDecision = async () => {
    ...
    try {
      const res = await sopDevelopmentService.reviewDecision({
        id: "sop-rec-00123",
        decision,
        comments,
      });
      ...
      <CardHeader className="pb-3 flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription className="text-xs">
            Cross-functional sign-offs: Process Owner, Dept. Head, Quality, EHS Manager, Compliance Officer, HR/Training, COO, CEO.
          </CardDescription>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
          ...
          <div className="xl:col-span-4 p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/40 space-y-4 text-xs">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={decision} onValueChange={(v) => setDecision(v as SopApprovalDecision)}>
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                ...
                rows={4}
                placeholder="Enter sign-off comments, ISO compliance notes, or training requirements..."
