...
import type { WorkInstructionApprovalDecision, WorkInstructionFormInput, WorkInstructionReviewer } from "@/services/types";
import { workInstructionDevelopmentService } from "@/services/workInstructionDevelopmentService";
import { toast } from "sonner";

export function WorkInstructionApprovalSection({
  form,
  reviewers,
}: {
  ...
  const [decision, setDecision] = useState<WorkInstructionApprovalDecision>("Approved");
  ...
  const handleApplyDecision = async () => {
    ...
    try {
      const res = await workInstructionDevelopmentService.reviewDecision({
  ...
  return (
    <Card className="border-border/80 shadow-xs bg-white dark:bg-slate-900">
      <CardHeader className="pb-3 flex flex-row items-center justify-between">
        <div>
          <CardTitle className="text-base font-bold">Multi-Level Review & Executive Approval</CardTitle>
          <CardDescription className="text-xs">
            Cross-functional sign-offs: Process Engineer, Manufacturing, Quality, EHS, Training Manager, Plant Head, COO, CEO.
          </CardDescription>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
          ...
          <div className="xl:col-span-4 p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/40 space-y-4 text-xs">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={decision} onValueChange={(v) => setDecision(v as WorkInstructionApprovalDecision)}>
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                ...
                rows={4}
                placeholder="Enter sign-off comments, quality notes, or training requirements..."
