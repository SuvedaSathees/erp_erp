...
import type { WorkInstructionAttachment } from "@/services/types";
import { workInstructionDevelopmentService } from "@/services/workInstructionDevelopmentService";
import { toast } from "sonner";

export function WorkInstructionAttachmentManager({
  attachments,
  onAttachmentsChange,
}: {
  attachments: WorkInstructionAttachment[];
  onAttachmentsChange?: (newAtts: WorkInstructionAttachment[]) => void;
}) {
  const [items, setItems] = useState<WorkInstructionAttachment[]>(attachments);
  const [isDragOver, setIsDragOver] = useState(false);
  const [selectedVersionDoc, setSelectedVersionDoc] = useState<WorkInstructionAttachment | null>(null);
  const [previewDoc, setPreviewDoc] = useState<WorkInstructionAttachment | null>(null);
  ...
  const handleFileUpload = async (files: FileList | null) => {
    ...
    try {
      const newAtt = await workInstructionDevelopmentService.uploadAttachment({
        name: file.name,
        type: file.type,
        size: file.size,
        documentType: "Instruction Attachment",
      });
  ...
  return (
    <Card className="border-border/80 shadow-xs bg-white dark:bg-slate-900 rounded-xl">
      <CardHeader className="pb-3 flex flex-row items-center justify-between">
        <div>
          <CardTitle className="text-base font-bold">Document & Reference Attachments</CardTitle>
          <CardDescription className="text-xs">
            Standard operating procedures (SOP), CAD drawings, mp4 assembly videos, control plans & training guides.
          </CardDescription>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        >
          <input
            type="file"
            id="workInstructionFileInput"
            className="hidden"
            onChange={(e) => handleFileUpload(e.target.files)}
          />
          <label htmlFor="workInstructionFileInput" className="cursor-pointer flex flex-col items-center justify-center gap-2">
            ...
            <div>
              ...
              <span className="text-[11px] text-muted-foreground block mt-0.5">
                Supported formats: .pdf, .mp4, .vsdx, .png, .jpeg, .xlsx (Max 150MB)
              </span>
            </div>
          </label>
        </div>
        ...
        <Dialog open={Boolean(previewDoc)} onOpenChange={() => setPreviewDoc(null)}>
          <DialogContent className="max-w-xl">
            ...
            <div className="p-4 rounded-lg bg-slate-900 text-slate-100 font-mono text-xs max-h-72 overflow-y-auto space-y-2">
              <p className="text-emerald-400 font-bold">=== WORK INSTRUCTION CONTROLLED DOCUMENT ===</p>
              ...
              <div className="mt-3 p-3 rounded bg-slate-800/80 text-slate-300 font-sans text-xs">
                This controlled assembly reference document specifies the approved tooling, torque parameters, and quality inspection criteria for Shop Floor Operation OP-20.
              </div>
            </div>
