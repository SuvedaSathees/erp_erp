...
interface LeanHeaderProps {
  ...
  onPrint?: () => void;
  onArchive?: () => void;
  onCloneFollowup?: () => void;
}

export const LeanHeader: React.FC<LeanHeaderProps> = ({
  record,
  onSaveDraft,
  onSubmitForApproval,
  onExportPdf,
        ...
        <div className="space-y-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-[10px] font-bold text-muted-foreground bg-muted/60 px-1.5 py-0.5 rounded border border-border/50 font-mono">
              {record.leanProjectNumber}
            </span>
          </div>

          <div className="flex items-center gap-3 text-xs text-muted-foreground">
            <span>Plant: <strong className="text-foreground">{record.plant}</strong></span>
            <span>•</span>
