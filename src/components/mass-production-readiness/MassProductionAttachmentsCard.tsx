...
import { FileText, Download, Link2, Upload } from "lucide-react";
import { Tooltip, TooltipTrigger, TooltipContent, TooltipProvider } from "@/components/ui/tooltip";
import type { AttachmentItem } from "@/lib/mass-production-readiness/types";
import { toast } from "sonner";

interface MassProductionAttachmentsCardProps {
  attachments: AttachmentItem[];
  onJumpToAttachmentsTab?: () => void;
  onUploadAttachment?: (file: File) => void;
}

export const MassProductionAttachmentsCard: React.FC<MassProductionAttachmentsCardProps> = ({
  attachments,
  onJumpToAttachmentsTab,
  onUploadAttachment,
}) => {
    ...
    <TooltipProvider>
      <div className="bg-card text-card-foreground border border-border rounded-lg p-4 shadow-sm space-y-3">
          ...
          {attachments.slice(0, 6).map((item) => {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 truncate">
                  ...
                  <div className="truncate">
                    ...
                    <span className="text-[10px] text-muted-foreground block">
                      {item.uploadedAt} • {item.fileSize || "2.1 MB"}
                    </span>
                  </div>
                </div>
                ...
                <button
                  onClick={() => {
                    const content = `MASS PRODUCTION READINESS ATTACHMENT: ${item.filename}\nUploaded: ${item.uploadedAt}\nFile Size: ${item.fileSize || "2.1 MB"}\nSource: ${item.source}\nStatus: Verified`;
        ...
        <div className="border-t border-border pt-2 text-right">
          <button
            onClick={onJumpToAttachmentsTab}
