...
function FundraisingDevelopmentPage() {
  ...
  const saveDraftMutation = useMutation({
    mutationFn: (input: any) => fundraisingService.saveDraft(input, loadedRecord?.id),
    onSuccess: (updated: any) => {
      queryClient.setQueryData(["fundraising-development"], updated);
  ...
  const [activityHistory, setActivityHistory] = useState(formData.activityHistory ?? []);
