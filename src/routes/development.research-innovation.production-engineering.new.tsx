...
Production Engineer: ${record.productionEngineer.name}
...
Process Dev Link: ${record.linkedProcessDevelopment.code}
...
READINESS SCORES:
-----------------------------------------------------
Overall Production Readiness: ${record.overallProductionReadiness}%
Design Readiness Score: ${record.designReadinessScore}%
Resource Readiness Score: ${record.resourceReadinessScore}%
Validation Score: ${record.validationScore}%
Quality & Safety Score: ${record.qualityScore}%
Performance Score: ${record.performanceScore}%

PROCESS PARAMETERS & CYCLE TIME:
-----------------------------------------------------
Standard Cycle Time: ${record.standardCycleTime} Min/Unit
...
-----------------------------------------------------
Planned Output: ${record.plannedOutput} Units/Month
OEE Target: ${record.oeeTarget}%
Yield Target: ${record.yieldTarget}%
Throughput Target: ${record.throughputTarget} Units/Hour
...
=====================================================`;
  ...
  const processDocuments = [
    { label: "Manufacturing Routing Sheet", filename: record.routingSheet, type: "PDF", size: "3.4 MB" },
    { label: "Operation Sequence Diagram", filename: record.operationSequence, type: "PDF", size: "2.8 MB" },
    { label: "Workstation Layout CAD Blueprint", filename: record.workstationLayout, type: "DWG", size: "8.6 MB" },
    ...
    >
      <div className="p-4 sm:p-6 space-y-5">
        ...
        <div className="space-y-5">
          ...
          <Card className="border-border/80 shadow-xs bg-white dark:bg-slate-900">
            <CardHeader className="pb-3 flex flex-row items-center justify-between">
              ...
              <Badge className="bg-emerald-50 text-emerald-700 border-emerald-200 text-xs font-semibold">
                Resource Score: {record.resourceReadinessScore}/100
              </Badge>
            </CardHeader>
          ...
          {/* Section 4: Capacity, Yield & Resource Requirements (3-Column Grid) */}
            ...
            <Card className="border-border/80 shadow-xs bg-white dark:bg-slate-900">
              ...
              <CardContent className="space-y-3 text-xs">
                <div>
                  ...
                  <span className="font-bold text-foreground text-sm font-mono">{record.plannedOutput.toLocaleString()} Units/Mo</span>
                </div>
                <div>
                  <span className="text-muted-foreground block text-[10px]">Standard Cycle Time</span>
                  <span className="font-bold text-foreground text-sm font-mono">{record.standardCycleTime} Min / Unit</span>
                </div>
                <div>
                  <span className="text-muted-foreground block text-[10px]">Throughput Target</span>
                  <span className="font-bold text-emerald-600 text-sm font-mono">{record.throughputTarget} Units / Hour</span>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-border/80 shadow-xs bg-white dark:bg-slate-900">
              ...
              <CardContent className="space-y-3 text-xs">
                <div>
                  ...
                  <span className="font-bold text-foreground text-sm font-mono">{record.oeeTarget}%</span>
                </div>
                <div>
                  <span className="text-muted-foreground block text-[10px]">Yield Target</span>
                  <span className="font-bold text-emerald-600 text-sm font-mono">{record.yieldTarget}%</span>
                </div>
                <div>
                  <span className="text-muted-foreground block text-[10px]">Process Validation</span>
                  <span className="font-semibold text-foreground">{record.processValidationStatus}</span>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-border/80 shadow-xs bg-white dark:bg-slate-900">
              ...
              <CardContent className="space-y-3 text-xs">
                <div>
                  ...
                  <span className="font-bold text-foreground text-sm font-mono">{record.machinesRequired} Dedicated Units</span>
                </div>
                <div>
                  <span className="text-muted-foreground block text-[10px]">Tooling & Fixtures Required</span>
                  <span className="font-bold text-foreground text-sm font-mono">{record.toolingRequired} Tooling Sets</span>
                </div>
                <div>
                  <span className="text-muted-foreground block text-[10px]">Process Parameters Spec</span>
                  <span className="font-semibold text-foreground">{record.processParameters}</span>
                </div>
              </CardContent>
            </Card>
          </div>

          {/* Section 5: Process PFMEA Controls Table */}
          <Card className="border-border/80 shadow-xs bg-white dark:bg-slate-900">
            <CardHeader className="pb-3 flex flex-row items-center justify-between">
              <div>
                  ...
                  <table className="w-full text-left border-collapse text-xs">
                    <thead>
                      <tr className="bg-muted/50 border-b border-border text-[11px] font-bold text-muted-foreground uppercase tracking-wider">
                        ...
                        <th className="py-3 px-4 text-center font-semibold whitespace-nowrap">S</th>
                        <th className="py-3 px-4 text-center font-semibold whitespace-nowrap">O</th>
                        <th className="py-3 px-4 text-center font-semibold whitespace-nowrap">D</th>
                        <th className="py-3 px-4 text-center font-semibold whitespace-nowrap">RPN</th>
                        <th className="py-3 px-4 font-semibold whitespace-nowrap">Poka-Yoke & Action</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-border/60">
                      {pfmeaItems.map((pf) => (
                        <tr key={pf.id} className="hover:bg-muted/30 transition-colors">
                          ...
                          <td className="py-3 px-4 text-center font-mono">{pf.severity}</td>
                          <td className="py-3 px-4 text-center font-mono">{pf.occurrence}</td>
                          <td className="py-3 px-4 text-center font-mono">{pf.detection}</td>
